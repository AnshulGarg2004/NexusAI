...
import { Coins, LogOut, Menu, MessageSquare, PanelLeftIcon, PanelRight, PanelRightIcon, PenSquare, Plus, User, X } from "lucide-react"
...
function SideBar() {
    ...
    return (
        <>
            <button onClick={() => setMobileOpen(true)} className='lg:hidden fixed top-3.5 left-4 z-50  items-center justify-center w-8 h-8 rounded-lg bg-[#0d0f14] border border-white/[0.06] text-slate-400 hover:text-slate-200 transition-colors duration-150 cursor-pointer'>
                <Menu size={14} />
            </button>
            ...
            <div className={`fixed lg:static inset-y-0 left-0 z-50 w-[270px] h-screen shrink-0 bg-[#0d0f14] border-r border-white/[0.06] transition-transform duration-300 ${mobileOpen ? " translate-x-0" : " -translate-x-full lg:translate-x-0"}`}>
                ...
                <div className='flex flex-col h-full'>
                    <div className='flex items-center gap-2.5 px-4 py-4 border-b border-white/[0.06]'>
                        ...
                        <button onClick={() => setMobileOpen(false)} className=' lg:hidden flex items-center justify-center w-7 h-7 rounded-lg text-slate-500
